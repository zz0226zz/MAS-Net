...
from model.masnet import Masnet

parser = argparse.ArgumentParser()
parser.add_argument('--volume_path', type=str,
                    default='/data/zz/seg/data/Synapse/test_vol_h5', help='root dir for validation volume data')  # for acdc volume_path=root_dir
...
parser.add_argument('--model_name', type=str,
                    default="masnet", help='the name of network')
...
def inference_ACDC(args, model, test_save_path=None):
    db_test = args.Dataset(base_dir=args.volume_path, split="test_vol", list_dir=args.list_dir)
    testloader = DataLoader(db_test, batch_size=1, shuffle=False, num_workers=1)
    logging.info("{} test iterations per epoch".format(len(testloader)))
    model.eval()
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        image, label, case_name = sampled_batch["image"], sampled_batch["label"], sampled_batch['case_name'][0]
        metric_i = test_single_volume_ACDC(image, label, model, classes=args.num_classes, patch_size=[args.img_size, args.img_size],
                                      test_save_path=test_save_path, case=case_name, z_spacing=1)
        metric_list += np.array(metric_i)
        logging.info('idx %d case %s mean_dice : %f mean_hd95 : %f' % (i_batch, case_name, np.mean(metric_i, axis=0)[0], np.mean(metric_i, axis=0)[1]))
    metric_list = metric_list / len(db_test)
...
if __name__ == "__main__":
    ...
    dataset_config = {
        'Synapse': {
            ...
            "checkpoint_path" : './pth/{}_mas_{}_{}_{}/best.pth'.format(args.model_name, args.base_lr,
        ...
        'ACDC': {
            ...
            "checkpoint_path" : './pth/{}_ACDC_{}_{}_{}/epoch_99.pth'.format(args.model_name, args.base_lr,
                                                                              args.max_epochs, args.dataset),
    ...
    net.load_state_dict(torch.load(args.checkpoint_path, map_location=torch.device('cuda:0')))
    ...
    if args.dataset == 'ACDC':
        inference_ACDC(args, net, test_save_path)
