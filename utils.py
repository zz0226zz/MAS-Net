    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                # all3, all2, all1, outputs = net(input.repeat(1,3,1,1))
...
def test_single_volume_ACDC(image, label, net, classes, patch_size=[224, 224], test_save_path=None, case=None, z_spacing=1):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        for ind in range(image.shape[0]):
